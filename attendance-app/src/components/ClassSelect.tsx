...
export default function ClassSelect({
  ...
  async function load() {
    ...
    try {
      ...
      show("Unable to load classes.", "error");
    } finally {
      setLoading(false);
    }
  }
  ...
  async function create() {
    ...
    try {
      ...
      if (!r.ok) {
        setFormError(data.error ?? "Unable to create class.");
        return;
      }
      setItems((prev) => [...prev, data].sort((a, b) => a.name.localeCompare(b.name)));
      onChange(data.id); // auto-select newly created
      setModalOpen(false);
      setName("");
      show("Class added.", "success");
    } catch {
      setFormError("Unable to create class. Try again.");
    } finally {
      setCreating(false);
    }
  }
